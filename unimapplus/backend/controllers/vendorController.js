...
async function addMenuItem(req, res) {
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO menu_items (vendor_id, item_name, description, price, price_label, image_url, tags, prep_time, prep_time_unit, item_type, variants, toppings, allow_design_notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [vendorId, item_name, description || '', price, price_label || null, image_url, JSON.stringify(tagsArray), prep_time || 15, prep_time_unit || 'mins', item_type || 'food', variants || '[]', toppings || '[]', allowDesignNotes]
    );
...
async function updateMenuItem(req, res) {
  try {
    ...
    const { item_name, description, price, price_label, tags, is_available, prep_time, prep_time_unit, item_type, variants, toppings, allow_design_notes } = req.body;
