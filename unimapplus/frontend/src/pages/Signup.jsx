...
const VENDOR_CATEGORIES = [
  {
    value: "african_food",
    label: "African Food",
    desc: "Jollof, soups, stews, local dishes",
  },
  {
    value: "fast_food",
    label: "Fast Food",
    desc: "Burgers, shawarma, fried chicken",
  },
  {
    value: "snacks",
    label: "Snacks & Bites",
    desc: "Sandwiches, pies, small chops",
  },
  {
    value: "drinks",
    label: "Drinks & Beverages",
    desc: "Zobo, juices, smoothies, water",
  },
  {
    value: "bakery",
    label: "Bakery & Pastry",
    desc: "Bread, cakes, puff puff, chin chin",
  },
  {
    value: "rice_dishes",
    label: "Rice Dishes",
    desc: "Fried rice, ofada, coconut rice",
  },
  {
    value: "protein",
    label: "Proteins & Grills",
    desc: "Suya, grilled fish, peppered meat",
  },
  {
    value: "vegetarian",
    label: "Vegetarian",
    desc: "Salads, veggies, plant-based meals",
  },
  {
    value: "foodstuff",
    label: "Foodstuff & Grocery",
    desc: "Provisions, ingredients, condiments",
  },
      ...
      {[0, 1, 2, 3, 4, 5].map((i) => (
        <input
          ...
          style={{
            ...
            fontSize: 22,
            fontWeight: 800,
            border: `2px solid ${digits[i] ? TEAL : "#e4e6ef"}`,
            borderRadius: 12,
            outline: "none",
            fontFamily: "inherit",
            color: DARK,
            background: digits[i] ? "#e6fafa" : "#fff",
            transition: "all .15s",
          }}
        />
      ))}
...
export default function Signup() {
  ...
  async function sendOTP() {
    setOtpError("");
    if (!schoolEmail.trim())
      return setOtpError("Enter your school email first");
    setOtpLoading(true);
    try {
  ...
  async function handleSubmit(e) {
    ...
    if (isCACPath && !verifyFile)
      return setError(
        "Please upload your CAC certificate or vendor approval document",
      );
    ...
    try {
      ...
      if (data.success) {
        ...
        navigate(
          { student: "/student", vendor: "/vendor", driver: "/rider" }[
            data.user.role
          ],
        );
      }
    ...
    >
      ...
      >
        ...
        <label style={{ ...lbl, margin: 0 }}>
          School Email Verification <span style={{ color: "#e74c3c" }}>*</span>
        </label>
      </div>
      <p
        style={{
        ...
        <>
          <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            <input
              ...
              style={{
                ...inp,
                flex: 1,
                borderColor: otpSent ? TEAL : "#e4e6ef",
              }}
            ...
            <button
              ...
              style={{
                padding: "0 14px",
                background: otpCooldown > 0 ? "#e4e6ef" : accentColor,
                color: "#fff",
                border: "none",
                borderRadius: 12,
                fontWeight: 700,
          ...
          {otpSent && (
            <div style={{ marginTop: 8 }}>
              ...
              >
                Enter the 6-digit code sent to <strong>{schoolEmail}</strong>
              </p>
              ...
              <button
                ...
                style={{
                  width: "100%",
                  padding: "10px",
                  background: otpValue.length === 6 ? accentColor : "#ccc",
                  color: "#fff",
                  border: "none",
                  borderRadius: 12,
                  fontWeight: 700,
  ...
  const NINBlock = () => (
    ...
    >
      ...
      >
        <span style={{ fontSize: 20 }}>🪪</span>
        <label style={{ ...lbl, margin: 0 }}>
          NIN Number <span style={{ color: "#e74c3c" }}>*</span>
        </label>
      </div>
      <p
        style={{
      ...
      >
        Enter your 11-digit National Identification Number (NIN). This is
        verified in real-time through the NIMC registry.
      </p>
      <input
        type="text"
        inputMode="numeric"
      ...
      >
        <span style={{ fontSize: 11, color: "#7a7a9a" }}>
          Verified via Prembly IdentityPass
        </span>
        <span
          style={{
            fontSize: 11,
            color: ninNumber.length === 11 ? TEAL : "#7a7a9a",
            fontWeight: 700,
          }}
        >
          {ninNumber.length}/11
      ...
      >
        🔒 Your NIN is used only for identity verification and is not stored
        after the check.
      </div>
    </div>
  );
  ...
  const docConfigs = {
    student_id: {
      label: "Student ID Card or Admission Letter",
      hint: "Upload a clear photo. Your name must match exactly as entered above.",
    },
    cac: {
      label: "CAC Certificate or Vendor Approval Letter",
      hint: "Upload your CAC business registration or a school authority approval letter.",
    },
  };
    ...
    !docCfg ? null : (
      ...
      >
        ...
        >
          ...
          <label style={{ ...lbl, margin: 0 }}>
            {docCfg.label} <span style={{ color: "#e74c3c" }}>*</span>
          </label>
        </div>
        <p
          style={{
        ...
        <label
          ...
          style={{
            ...
            gap: 8,
            padding: "10px 14px",
            background: verifyFile ? "#e6fafa" : "#fff",
            border: `1.5px dashed ${verifyFile ? TEAL : "#c7d7ff"}`,
            borderRadius: 10,
            cursor: "pointer",
            fontSize: 13,
            fontWeight: 600,
    ...
    >
      ...
      >
        ...
        <form
          ...
          style={{ padding: "24px 28px", maxHeight: "78vh", overflowY: "auto" }}
        >
          {/* Role tabs */}
          <div
            style={{
            ...
            {["student", "vendor", "driver"].map((r) => (
              <button
                ...
                style={{
                  ...
                  boxShadow:
                    form.role === r ? "0 1px 4px rgba(0,0,0,0.1)" : "none",
                  transition: "all .15s",
                  fontFamily: "inherit",
                }}
              >
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={lbl}>
              {isVendor ? "Eatery / Business Name" : "Full Name"}
            </label>
            <input
              type="text"
              required
            ...
            {isVendorOrRider && (
              <p style={{ margin: "4px 0 0", fontSize: 11, color: "#7a7a9a" }}>
                ⚠️ Must match your name exactly as it appears on your
          ...
          {isDriver && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                {[
                  ["student", "Student Rider", "I am a student at this school"],
                  ["independent", "Independent Rider", "I am not a student"],
                ].map(([val, label, desc]) => (
                  <div
                    key={val}
                    onClick={() => {
                      setForm((p) => ({ ...p, rider_type: val }));
                      resetVerification();
                    }}
                    style={{
                      flex: 1,
                      padding: "10px 12px",
                      borderRadius: 12,
                      border: `2px solid ${form.rider_type === val ? "#2D6A4F" : "#e4e6ef"}`,
                      background: form.rider_type === val ? "#f0faf4" : "#fff",
                      cursor: "pointer",
                      textAlign: "center",
                    }}
                  >
                    <div
                      style={{
                        fontSize: 13,
                        fontWeight: 700,
                        color: form.rider_type === val ? "#2D6A4F" : DARK,
                      }}
                    >
                      {label}
                    </div>
                    <div
                      style={{ fontSize: 10, color: "#7a7a9a", marginTop: 2 }}
                    >
                      {desc}
                    </div>
                  </div>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                {[
                  [
                    "student",
                    "Student Vendor",
                    "I am a student selling on campus",
                  ],
                  [
                    "business",
                    "School Buka / Business",
                    "Registered business or school canteen",
                  ],
                ].map(([val, label, desc]) => (
                  <div
                    key={val}
                    onClick={() => {
                      setForm((p) => ({ ...p, vendor_type: val }));
                      resetVerification();
                    }}
                    style={{
                      flex: 1,
                      padding: "10px 12px",
                      borderRadius: 12,
                      border: `2px solid ${form.vendor_type === val ? "#E87C2A" : "#e4e6ef"}`,
                      background: form.vendor_type === val ? "#fff7f0" : "#fff",
                      cursor: "pointer",
                      textAlign: "center",
                    }}
                  >
                    <div
                      style={{
                        fontSize: 12,
                        fontWeight: 700,
                        color: form.vendor_type === val ? "#E87C2A" : DARK,
                      }}
                    >
                      {label}
                    </div>
            ...
            <>
              <div style={{ marginBottom: 16 }}>
                <label style={lbl}>
                  ...
                  <span
                    style={{ fontSize: 11, color: "#7a90a4", fontWeight: 400 }}
                  >
                    (optional)
                  </span>
                </label>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          fontFamily: "inherit",
                          cursor: "pointer",
                          border: `1.5px solid ${sel ? "#E87C2A" : "#e4e6ef"}`,
                          borderRadius: 12,
                          background: sel ? "#fff6ee" : "#fff",
                          transition: "all .15s",
                          outline: "none",
                        }}
                      >
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={lbl}>
                  Short Description{" "}
                  <span
                    style={{ fontSize: 11, color: "#7a7a9a", fontWeight: 400 }}
                  >
                    (optional)
                  </span>
                </label>
                <input
                  type="text"
                  placeholder="e.g. Best jollof rice on campus"
                  maxLength={100}
          ...
          {/* Password */}
          <div
            style={{ marginBottom: 14, marginTop: isVendorOrRider ? 16 : 0 }}
          >
